...
import { REAL_ESTATE_INTERESTS } from "../constants/homePageConstant";
import { FiChevronDown } from "react-icons/fi";
import { useSectionContext } from "../context/SectionContext";

export default function RealEstateInterestsSection() {
  const { expandedSection, toggleSection } = useSectionContext();
  const isExpanded = expandedSection === 'realEstate';

  const handleToggle = () => {
    toggleSection('realEstate');
  };

  return (
    <section id="realEstate" className="w-full flex flex-col items-center lg:py-16 py-8">
      <div className="w-full lg:max-w-6xl max-w-full">
        ...
        >
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:p-5 p-0">
            {REAL_ESTATE_INTERESTS.map((item, index) => {
              return (
                <div
                  key={item.title}
